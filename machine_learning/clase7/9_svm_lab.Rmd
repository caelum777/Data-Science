...
confucion <- table(true = dat[-train, "y"], 
      pred = predict(tune.out$best.model, newx = dat[-train, ]))

acurracy <- (confucion[1,1] + confucion[2,2]) / sum(confucion)
...
plot(roc.curve(scores.class0 = fitted, weights.class0=as.numeric(dat[-train, "y"]), curve=TRUE))
```
